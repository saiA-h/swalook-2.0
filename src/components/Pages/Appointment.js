...
function Appointment() {
  ...
  const handleAddAppointment = async e => {
    ...
    if (services.length === 0 || !bookingTime || !bookingDate || !/^(\+91)?[0-9]{10}$/.test(mobileNo)) {
      setDialogTitle('Error');
      setDialogMessage(!services.length ? 'Please select at least one service.' :
        !bookingTime ? 'Please select time.' :
        !bookingDate ? 'Please select date.' :
    ...
    try {
      ...
      setPopupMessage("Failed to add appointment!");
      setShowPopup(true);
            ...
            <form onSubmit={handleAddAppointment}>
              ...
              <div className='ba_con'>
                ...
                <div className="appointform-group" style={{ marginTop: '10px' }}>
                  ...
                  <input id="comments" type='text' className="appoint_inputs" placeholder='Enter Comments' onChange={e => setComments(e.target.value)} />
                </div>
